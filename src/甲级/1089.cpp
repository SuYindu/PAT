...
void InsertIterate(vector<int> &v, int pos) {
    for (int i = pos; i > 0; i--)
        if (v[i-1] > v[i]) swap(v[i], v[i-1]);
}

bool InsertSort(vector<int> &init, vector<int> &part) {
    vector<int> temp(init);
    for (int i = 1; i < init.size(); i++) {
        InsertIterate(temp, i);
        if (Equal(temp, part)) {
            InsertIterate(temp, ++i);
...
void MergeIterate(vector<int> &work, vector<int> &temp, int len) {
    copy(work.begin(), work.end(), temp.begin());
    for (int i = 0; i < work.size(); i += 2 * len)
        Merge(work, temp, i, i + len, len);
}

bool MergeSort(vector<int> &init, vector<int> &part) {
    vector<int> work(init), temp(init);
    for (int len = 1; len < init.size(); len *= 2) {
        MergeIterate(work, temp, len);
        if (Equal(work, part)) {
            MergeIterate(work, temp, len *= 2);
...
int main() {
    ...
    if (!InsertSort(init, part)) MergeSort(init, part);
    return 0;
}